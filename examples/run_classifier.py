...
def convert_examples_to_features(examples, label_list, max_seq_length,
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        assert len(arg3_mask) == max_seq_length
...
def main():
    ...
    if args.do_train:
        ...
        all_arg3_masks = torch.tensor([f.arg3_mask for f in train_features], dtype=torch.long)
        ...
        train_data = TensorDataset(
            all_input_ids, all_input_mask, all_segment_ids, all_label_ids, all_target_idxs, all_subj_masks, all_obj_masks, all_arg3_masks, all_adjustments
        )
        if args.local_rank == -1:
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                input_ids, input_mask, segment_ids, label_ids, target_idxs, subj_masks, obj_masks, arg3_masks, adjustments = batch

                # define a new function to compute loss values for both output_modes
                pi, mu, sigma = model(
                    input_ids, segment_ids, input_mask, labels=None, target_idx=target_idxs,
                    subj_mask=subj_masks, obj_mask=obj_masks, arg3_mask=arg3_masks,
                )
