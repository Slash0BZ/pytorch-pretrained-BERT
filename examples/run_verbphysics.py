...
class VerbPhysicsClassification(nn.Module):
    def __init__(self, num_labels, input_size):
        ...
        self.layer_norm = LayerNorm(input_size, eps=1e-12)
        self.classifier = nn.Linear(self.hidden_size * 2, num_labels)

    def forward(self, input_1s, input_2s, labels=None):
...
class Runner:
    ...
    class Instance:
        ...
        def __init__(self, input_1, input_2, label, num_label, input_size, agreement):
    ...
    def __init__(self, train_data, dev_data, test_data, embedding_file):
        ...
        self.model = VerbPhysicsClassification(self.num_label, self.input_size).to(self.device)
    ...
    def load_data_file(self, file_name):
        ...
        for line in [x.strip() for x in open(file_name).readlines()]:
            ...
            instance = self.Instance(
                ...
                label_map[label], self.num_label, self.input_size, int(group[5])
            )
    ...
    def train(self, epoch=5, batch_size=32):
        ...
        train_data_tensor = TensorDataset(
            all_train_input_1, all_train_input_2, all_train_labels
        )
        ...
        for _ in trange(epoch, desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                input_1s, input_2s, labels = batch

                logits = self.model(input_1s, input_2s, None)
    ...
    def eval(self, data, batch_size=8):
        ...
        eval_data = TensorDataset(all_test_input_1, all_test_input_2, all_test_labels)
        ...
        all_preds = []
        all_labels = []
        for input_1s, input_2s, labels in tqdm(eval_dataloader, desc="Evaluating"):
            input_1s = input_1s.to(self.device)
            input_2s = input_2s.to(self.device)
            labels = labels.to(self.device).detach().cpu().numpy()

            with torch.no_grad():
                logits = self.model(input_1s, input_2s, None).detach().cpu().numpy()
...
if __name__ == "__main__":
    runner = Runner(
        ...
        embedding_file="samples/verbphysics/train-5/obj_embedding.pkl"
    )
    runner.train(epoch=1000, batch_size=16)
