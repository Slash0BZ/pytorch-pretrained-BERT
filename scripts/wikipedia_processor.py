...
class TokenizationProcessor:
    ...
    def __init__(self):
        self.lines = [x.strip() for x in open("samples/wikipedia/raw_collection_contextsent.txt").readlines()]
        self.nlp = English()
        self.nlp.add_pipe(self.nlp.create_pipe('sentencizer'))
        self.f_out = open("samples/wikipedia/raw_collection_contextsent_tokenized.txt", "w")
        self.process()
...
# extractor.process_path("/shared/wikipedia/processed/enwiki_with_links", "samples/wikipedia/raw_collection_contextsent.txt")
